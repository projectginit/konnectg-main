...
const CATEGORIES = [
  { icon: '🍽️', bg: '#F1E6FB', name: 'Restaurants & Food', count: 48 },
  { icon: '🏥', bg: '#E3F1FB', name: 'Healthcare', count: 32 },
  { icon: '📚', bg: '#E2F7EA', name: 'Education', count: 25 },
  { icon: '🛒', bg: '#E1F0FB', name: 'Grocery & Retail', count: 56 },
  { icon: '💇‍♀️', bg: '#FBE5EF', name: 'Beauty & Wellness', count: 19 },
  { icon: '🔧', bg: '#FCEBDB', name: 'Home Services', count: 41 },
]
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
            {CATEGORIES.map((c) => (
              <div
                key={c.name}
                onClick={() => navigate(`/listings?category=${encodeURIComponent(c.name)}`)}
              ...
              >
                <div className="mb-3.5 flex h-11 w-11 items-center justify-center rounded-xl text-xl" style={{ background: c.bg }}>
                  {c.icon}
                </div>
            ...
            {MERCHANTS.map((m) => (
              <div key={m.name} className="overflow-hidden rounded-2xl border border-purple-50 bg-white shadow-sm">
                <div className="flex h-32 items-start justify-end p-3.5" style={{ background: m.cover }}>
                  <span className="flex items-center gap-1 rounded-full bg-white px-3 py-1 text-xs font-bold text-emerald-600">✓ Verified</span>
                </div>
                <div className="p-5">
                  <h3 className="font-display font-bold text-ink">{m.name}</h3>
                  <p className="mb-2.5 text-sm text-ink-soft">{m.cat}</p>
                  <p className="mb-4 text-sm text-gold-dark">{m.stars} <span className="text-ink-soft">({m.reviews} reviews)</span></p>
          ...
          <div className="grid gap-6 sm:grid-cols-3">
            {OFFERS.map((o) => (
              <div key={o.title} className="rounded-2xl border border-purple-50 bg-white p-5">
                <span className="mb-3.5 inline-block rounded-full bg-gold px-3 py-1 text-xs font-bold text-purple-900">{o.pill}</span>
      ...
      <section className="px-6 py-16 text-center">
        <div className="mx-auto max-w-5xl">
          ...
          <div className="grid gap-10 sm:grid-cols-3">
            {STEPS.map((s) => (
              <div key={s.title}>
                <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl text-2xl" style={{ background: s.bg }}>
                  {s.icon}
                </div>
